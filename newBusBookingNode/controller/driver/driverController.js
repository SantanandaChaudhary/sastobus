...
exports.addBus = async (req, res) => {
  ...
  await sequelize.query(
    ...
    (id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,seat VARCHAR(255),side VARCHAR(255),status VARCHAR(255),busId INT REFERENCES buses(id) ON DELETE CASCADE ON UPDATE CASCADE,userId INT REFERENCES users(id) ON DELETE CASCADE ON UPDATE CASCADE )`,
    {
  ...
  for (var i = 0; i < req.body.seats; i++) {
    await sequelize.query(
      `INSERT INTO busSeats_${bus.id} (seat,side,status,busId) VALUES ('${busData[i].seatNumber}','${busData[i].side}','available','${bus.id}')`,
      {
...
exports.getBusSeatsSingle = async (req, res) => {
  const seats = await sequelize.query(
    `SELECT * FROM busSeats_${req.params.id} WHERE status='available'  `,
    {
      type: QueryTypes.SELECT,
    }
  );
